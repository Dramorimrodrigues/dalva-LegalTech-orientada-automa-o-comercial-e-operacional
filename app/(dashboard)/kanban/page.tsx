...
export default function KanbanPage() {
  ...
  useEffect(() => {
    fetch('/api/kanban')
      .then(r => r.json())
      .then(res => { if (res.success) setColumns(res.data.colunas); })
  ...
  const handleDrop = async (targetColumnId: string) => {
    ...
    await fetch('/api/kanban', {
      ...
      body: JSON.stringify({ leadId: draggedLead.id, newStatus: targetColumnId }),
    });
  };
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-dalva-text-muted mt-1">Arraste os cards para mover leads entre etapas do funil</p>
        </div>
        <div className="flex items-center gap-2">
          {Object.entries(NICHO_CONFIG).map(([key, config]) => (
            <span key={key} className="badge" style={{ background: config.corBg, color: config.cor }}>
      ...
      <div className="flex gap-4 overflow-x-auto pb-4 -mx-2 px-2">
        {columns.map(column => (
          <div
            key={column.id}
            className={`kanban-column transition-all duration-200 ${dragOverColumn === column.id ? 'ring-2 ring-dalva-gold/30 bg-dalva-gold-glow' : ''}`}
          ...
          >
            <div className="flex items-center gap-2 px-3 py-3 mb-1">
              ...
              <h3 className="text-sm font-semibold text-dalva-text-primary flex-1">{column.titulo}</h3>
              <span className="text-xs font-bold px-2 py-0.5 rounded-lg" style={{ background: `${column.cor}20`, color: column.cor }}>
                {column.leads.length}
              </span>
            </div>
            ...
            <div className="space-y-2 px-1.5 pb-2 min-h-[120px]">
              {column.leads.map(lead => {
                const nicho = NICHO_CONFIG[lead.nicho as keyof typeof NICHO_CONFIG];
                  ...
                  >
                    <div className="flex items-start gap-2 mb-2.5">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          <div className="w-7 h-7 rounded-lg flex items-center justify-center text-[10px] font-bold flex-shrink-0" style={{ background: nicho.corBg, color: nicho.cor }}>
                            {getInitials(lead.nome)}
                          </div>
                          <p className="text-sm font-medium text-dalva-text-primary truncate">{lead.nome}</p>
                        </div>
                      </div>
                    </div>
                    <div className="flex items-center gap-1.5 mb-2.5">
                      <span className="text-[10px] px-1.5 py-0.5 rounded-md font-medium" style={{ background: nicho.corBg, color: nicho.cor }}>
                        {nicho.icone} {nicho.label}
                      </span>
                      <span className="text-[10px] px-1.5 py-0.5 rounded-md font-bold" style={{ background: `${getScoreColor(lead.score)}15`, color: getScoreColor(lead.score) }}>
                        Score {lead.score}
                      </span>
                    </div>
                    <p className="text-xs text-dalva-text-muted line-clamp-2 mb-2.5">{lead.ultimaMensagem}</p>
