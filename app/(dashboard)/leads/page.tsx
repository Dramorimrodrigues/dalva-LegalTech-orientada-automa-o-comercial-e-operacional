...
import { NICHO_CONFIG, STATUS_CONFIG, NichoJuridico, LeadStatus } from '@/lib/types';
...
const NICHO_VALUES: readonly string[] = ['todos', 'consumidor', 'trabalhista', 'previdenciario'] as const;
const STATUS_VALUES: readonly string[] = ['todos', 'novo', 'qualificando', 'qualificado', 'contrato_enviado', 'convertido', 'descartado'] as const;
...
// Precisa de Suspense wrapper — Next.js 15
// ─────────────────────────────────────────────
function LeadsContent() {
  ...
  // Lê o parâmetro ?q= da URL (busca global do Header)
  useEffect(() => {
  ...
  const filteredLeads = useMemo(() => {
    ...
    if (search) {
      ...
      result = result.filter(
        (l) =>
          ...
          l.resumoCaso.toLowerCase().includes(q)
      );
    }

    // Nicho filter
    if (nichoFilter !== 'todos') {
      result = result.filter((l) => l.nicho === nichoFilter);
    }
    ...
    // Sort — usa id como desempate para ordenação estável (evita hydration mismatch)
    result.sort((a, b) => {
      let cmp = 0;
      switch (sortField) {
        case 'nome':
        ...
        case 'criadoEm':
          ...
          break;
        case 'atualizadoEm':
          cmp = new Date(a.atualizadoEm).getTime() - new Date(b.atualizadoEm).getTime();
          break;
      }
      // Desempate pelo id garante ordem determinística (corrige hydration mismatch)
      if (cmp === 0) cmp = a.id.localeCompare(b.id);
      return sortDir === 'asc' ? cmp : -cmp;
    });

    return result;
  }, [search, nichoFilter, statusFilter, sortField, sortDir]);
      ...
      <div className="glass-card p-4">
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className={cn(
              'flex items-center gap-2 px-3.5 py-2.5 rounded-xl text-sm border transition-colors',
              showFilters
                ? 'bg-dalva-gold-muted border-dalva-gold/20 text-dalva-gold'
                : 'bg-dalva-surface border-dalva-border text-dalva-text-secondary hover:border-dalva-border-hover'
            )}
          >
        ...
        {showFilters && (
          <div className="flex items-center gap-3 mt-3 pt-3 border-t border-dalva-border animate-fade-in">
            ...
            <div className="flex items-center gap-2">
              ...
              <select
                value={nichoFilter}
                onChange={(e) => { const v = e.target.value; if (NICHO_VALUES.includes(v)) setNichoFilter(v as NichoJuridico | 'todos'); }}
            ...
            <div className="flex items-center gap-2">
              ...
              <select
                value={statusFilter}
                onChange={(e) => { const v = e.target.value; if (STATUS_VALUES.includes(v)) setStatusFilter(v as LeadStatus | 'todos'); }}
                className="px-3 py-1.5 rounded-lg text-xs bg-dalva-surface border border-dalva-border text-dalva-text-primary outline-none focus:border-dalva-gold/30"
              >
                <option value="todos">Todos</option>
            ...
            {(nichoFilter !== 'todos' || statusFilter !== 'todos') && (
              <button
                onClick={() => { setNichoFilter('todos'); setStatusFilter('todos'); }}
              ...
              <tr className="border-b border-dalva-border">
                <th className="text-left px-5 py-3.5">
                  <button
                    ...
                    className="flex items-center gap-1 text-xs font-semibold text-dalva-text-muted uppercase tracking-wider hover:text-dalva-text-secondary transition-colors"
                  >
                    Lead
                ...
                <th className="text-left px-5 py-3.5">
                  <span className="text-xs font-semibold text-dalva-text-muted uppercase tracking-wider">
                    Nicho
                  </span>
                </th>
                <th className="text-left px-5 py-3.5">
                ...
                <th className="text-center px-5 py-3.5">
                  <button
                    ...
                    className="flex items-center gap-1 text-xs font-semibold text-dalva-text-muted uppercase tracking-wider hover:text-dalva-text-secondary transition-colors mx-auto"
                  >
                    Score
                    <ArrowUpDown className="w-3 h-3" />
                  </button>
            ...
            <tbody>
              {filteredLeads.map((lead) => {
                ...
                return (
                  <tr
                    key={lead.id}
                    className="border-b border-dalva-border/50 hover:bg-white/[0.02] transition-colors group"
                  >
                    {/* Lead */}
                    ...
                    <td className="px-5 py-3.5">
                      <span
                        className="badge text-[10px]"
                        style={{ background: status.corBg, color: status.cor }}
                      >
                    ...
                    {/* Updated — suppressHydrationWarning corrige mismatch de data/hora */}
                    <td className="px-5 py-3.5">
                      <span
                        className="text-xs text-dalva-text-muted"
                    ...
                    <td className="px-5 py-3.5">
                      <div className="flex items-center justify-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button
                          ...
                          title="Ver conversa"
                        >
                          <MessageSquare className="w-3.5 h-3.5 text-dalva-text-muted" />
                        </button>
                        <button
                          className="p-1.5 rounded-lg hover:bg-white/[0.06] transition-colors"
                          title="Ver detalhes"
                        >
                          <Eye className="w-3.5 h-3.5 text-dalva-text-muted" />
                        </button>
                        <button
                          className="p-1.5 rounded-lg hover:bg-white/[0.06] transition-colors"
                          title="Mais opções"
                        >
                          <MoreHorizontal className="w-3.5 h-3.5 text-dalva-text-muted" />
                        </button>
                      </div>
                    </td>
                  </tr>
...
// ─────────────────────────────────────────────
// Export da página com Suspense obrigatório
// (Next.js 15 exige para useSearchParams)
// ─────────────────────────────────────────────
